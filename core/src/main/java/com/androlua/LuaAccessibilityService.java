...
public class LuaAccessibilityService extends AccessibilityService {
    ...
    public interface AccessibilityServiceCallbacks {
        void onAccessibilityEvent(LuaAccessibilityService service, AccessibilityEvent event);

        void onInterrupt(LuaAccessibilityService service);

        void onServiceConnected(LuaAccessibilityService service);

        void onCreate(LuaAccessibilityService service);

        boolean onKeyEvent(LuaAccessibilityService service, KeyEvent event);

        void onDestroy(LuaAccessibilityService service);

        void onConfigurationChanged(LuaAccessibilityService service, Configuration newConfig);
    }
    ...
    public boolean isClickable(AccessibilityNodeInfo source) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return source.getActionList().contains(AccessibilityNodeInfo.AccessibilityAction.ACTION_CLICK);
        } else {
            return (source.getActions() & AccessibilityNodeInfo.ACTION_CLICK) != 0;
        }
    }
