 ...
 * @param successors       {@code non-null;} full list of successors that this block may
 *                         branch to
 ...
 */
public record BasicBlock(int label, InsnList insns, IntList successors,
                         int primarySuccessor) implements LabeledItem {
    /**
    ...
    public BasicBlock {
        ...
        for (int i = sz - 2; i >= 0; i--) {
            ...
            if (one.branchingness() != Rop.BRANCH_NONE) {
        ...
        if (lastInsn.getOpcode().branchingness() == Rop.BRANCH_NONE) {
