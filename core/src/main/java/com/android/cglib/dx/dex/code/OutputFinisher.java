...
public final class OutputFinisher {
    ...
    private int calculateReservedCount(Dop[] opcodes) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (newOpcode == null) {
                ...
                BitSet compatRegs = expandedOp.format().compatibleRegs(insn);
    ...
    private Dop findOpcodeForInsn(DalvInsn insn, Dop guess) {
        ...
        while (guess != null) {
            if (guess.format().isCompatible(insn)) {
                break;
            }
    ...
    private ArrayList<DalvInsn> performExpansion(Dop[] opcodes) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (currentOpcode != null) {
                ...
                BitSet compatRegs =
                        currentOpcode.format().compatibleRegs(insn);
    ...
    private boolean fixBranches() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (opcode.format().branchFits(target)) {
                continue;
            }

            if (opcode.family() == Opcodes.GOTO) {
