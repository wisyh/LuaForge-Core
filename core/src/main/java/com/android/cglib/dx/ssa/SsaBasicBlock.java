...
public final class SsaBasicBlock {
    ...
    public static SsaBasicBlock newFromRop(RopMethod rmeth,
        ...
        SsaBasicBlock result =
                new SsaBasicBlock(basicBlockIndex, bb.label(), parent);
        InsnList ropInsns = bb.insns();
        ...
        result.predecessors = SsaMethod.bitSetFromLabelList(
                ropBlocks,
                rmeth.labelToPredecessors(bb.label()));

        result.successors
                = SsaMethod.bitSetFromLabelList(ropBlocks, bb.successors());
        ...
        if (result.successorList.size() != 0) {
            int primarySuccessor = bb.primarySuccessor();
    ...
    public void replaceLastInsn(Insn insn) {
        if (insn.getOpcode().branchingness() == Rop.BRANCH_NONE) {
