...
public final class RopTranslator {
    ...
    private static boolean calculateParamsAreInOrder(RopMethod method,
        ...
        method.getBlocks().forEachInsn(new Insn.BaseVisitor() {
            ...
            public void visitPlainCstInsn(PlainCstInsn insn) {
                if (insn.getOpcode().opcode() == RegOps.MOVE_PARAM) {
                    int param =
    ...
    private void outputBlock(BasicBlock block, int nextLabel) {
        ...
        block.insns().forEach(translationVisitor);
        ...
        int succ = block.primarySuccessor();
        ...
        if ((succ >= 0) && (succ != nextLabel)) {
            ...
            if ((lastRop.branchingness() == Rop.BRANCH_IF) &&
        ...
        for (int label = method.getFirstLabel();
            ...
            for (; ; ) {
                ...
                for (int i = 0; i < psz; i++) {
                    ...
                    if (pred.primarySuccessor() == label) {
                        // Found one!
                        label = predLabel;
            ...
            while (label != -1) {
                ...
                int preferred = preferredBlock.label();
                int primary = one.primarySuccessor();
                ...
                if (Bits.get(workSet, preferred)) {
                     ...
                     */
                    IntList successors = one.successors();
    ...
    private class TranslationVisitor implements Insn.Visitor {
        ...
        public void visitPlainInsn(PlainInsn insn) {
            ...
            if (rop.opcode() == RegOps.MARK_LOCAL) {
                /*
            ...
            if (rop.opcode() == RegOps.MOVE_RESULT_PSEUDO) {
                // These get skipped
                return;
            }
            ...
            switch (rop.branchingness()) {
                ...
                case Rop.BRANCH_IF: {
                    int target = block.successors().get(1);
        ...
        public void visitPlainCstInsn(PlainCstInsn insn) {
            ...
            Dop opcode = RopToDop.dopFor(insn);
            Rop rop = insn.getOpcode();
            int ropOpcode = rop.opcode();
            DalvInsn di;

            if (rop.branchingness() != Rop.BRANCH_NONE) {
                throw new RuntimeException("shouldn't happen");
            }
        ...
        public void visitSwitchInsn(SwitchInsn insn) {
            ...
            IntList successors = block.successors();
            int casesSz = cases.size();
            int succSz = successors.size();
            int primarySuccessor = block.primarySuccessor();
        ...
        private RegisterSpec getNextMoveResultPseudo() {
            int label = block.primarySuccessor();
            ...
            Insn insn
                    = method.getBlocks().labelToBlock(label).insns().get(0);

            if (insn.getOpcode().opcode() != RegOps.MOVE_RESULT_PSEUDO) {
                return null;
            } else {
        ...
        public void visitThrowingCstInsn(ThrowingCstInsn insn) {
            ...
            if (rop.branchingness() != Rop.BRANCH_THROW) {
                throw new RuntimeException("shouldn't happen");
            }
            ...
            if (rop.isCallLike()) {
                ...
                if ((rop.opcode() == RegOps.NEW_ARRAY) &&
                        (opcode.opcode() != Opcodes.NEW_ARRAY)) {
                    /*
