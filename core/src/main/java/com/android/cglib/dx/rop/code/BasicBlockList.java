...
public final class BasicBlockList extends LabeledList {
    ...
    public int getEffectiveInstructionCount() {
        ...
        for (int i = 0; i < sz; i++) {
            BasicBlock one = (BasicBlock) getOrNull0(i);
            if (one != null) {
                InsnList insns = one.insns();
                int insnsSz = insns.size();

                for (int j = 0; j < insnsSz; j++) {
                    Insn insn = insns.get(j);

                    if (insn.getOpcode().opcode() != RegOps.MARK_LOCAL) {
                        result++;
                    }
                }
            }
    ...
    public boolean catchesEqual(BasicBlock block1, BasicBlock block2) {
        ...
        IntList succ1 = block1.successors();
        IntList succ2 = block2.successors();
        int size = succ1.size(); // Both are guaranteed to be the same size.

        int primary1 = block1.primarySuccessor();
        int primary2 = block2.primarySuccessor();
