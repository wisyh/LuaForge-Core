...
public final class CatchStructs {
    ...
    public void encode(DexFile file) {
        ...
        for (int i = 0; i < size; i++) {
            handlerOffsets.put(table.get(i).handlers(), null);
        }
        ...
        for (Map.Entry<CatchHandlerList, Integer> mapping :
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                out.writeUleb128(
                        typeIds.indexOf(entry.exceptionType()));
                out.writeUleb128(entry.handler());
            }

            if (catchesAll) {
                out.writeUleb128(list.get(listSize).handler());
            }
        }
    ...
    public void writeTo(DexFile file, AnnotatedOutput out) {
        ...
        for (int i = 0; i < tableSize; i++) {
            ...
            out.writeShort(handlerOffsets.get(one.handlers()));
        }
    ...
    private void annotateEntries(String prefix, PrintWriter printTo,
        ...
        for (int i = 0; i < size; i++) {
            ...
            CatchHandlerList handlers = entry.handlers();
            String s1 = subPrefix + "try " + Hex.u2or4(entry.start())
                    + ".." + Hex.u2or4(entry.end());
